...
class BaiduBCS {
	...
	public function list_bucket($opt = array()) {
		...
		$this->log ( $response->isOK () ? "Lst bucket success!" : "Lst bucket failed! Response: [" . $response->body . "]", $opt );
		return $response;
	}
	...
	public function list_object($bucket, $opt = array()) {
		...
		$response = $this->authenticate ( $opt );
		$this->log ( $response->isOK () ? "Lst object success!" : "Lit object failed! Response: [" . $response->body . "]", $opt );
		return $response;
	}
