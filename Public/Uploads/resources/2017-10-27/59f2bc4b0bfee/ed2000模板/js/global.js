function AdOverTime(TimeValue){ 
	TimeValue = TimeValue.replace(/-/g,"/");
	if(new Date(TimeValue) > new Date()) {return true;}
}
...
//判断域名
//if(location.toString().indexOf("ed2000.com") <= -1){document.location.href="http://www.ed2000.com";}
...
function ShowMagnet(MagnetURI){
	...
	if (MagnetDn ==undefined || MagnetDn ==""){
		...
		XunLeiFilename="ED2000.COM";
	}else{
		MagnetURI = MagnetURI.replace(MagnetDn,MagnetDn+"(ED2000.COM)");
		XunLeiFilename=MagnetDn
	}

	document.write("<tr class=CommonListCell><td width=870><img src='http://img.ed2000.com/Images/magnet.png' border=0 /> <a href=\""+MagnetURI+"\">"+MagnetDn+"</a></td><td width=80 align=center>"+FormatSize(MagnetRequest("xl",MagnetURI))+"</td></tr>")
}
...
function voteComment(CommentID,vote){
	...
	document.getElementById(CommentSpanID).innerHTML = "<img src=http://img.ed2000.com/Images/loading.gif>";
}
...
function FormatSize(ByteSize){
	if (ByteSize >= 1099511627776){
		...
		ByteSize="<img src='http://img.ed2000.com/Images/magnet-icon-14w-14h.gif'>";
	}
	return ByteSize; 
}
