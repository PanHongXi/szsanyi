$(function () {
    ...
    if ($(".emotion").length > 0) {
        $(".emotion").click(function () {
            ...
            $("#smileBoxOuter").css({"left": left, "top": top + 20}).show().attr("data-id", id)
        });
        ...
        $(".comment_blockquote").hover(function () {
            ...
            $(this).find(".comment_action_sub").css({"visibility": "visible"})
        }, function () {
            $(".comment_action_sub").css({"visibility": "hidden"})
        })
    }
    ...
    if ($("#detail-page").length > 0) {
        ...
        $("#comment_wrap").on("click", ".pager a", function () {
            ...
            $.get(getUrl("Box/comments"), {p: page, id: id, totalnum: totalnum, mtype: mtype}, function (data) {
    ...
    if ($("#qrcode_box").length > 0) {
        $("#btn_demo").hover(function () {
            ...
            var top = $(this).offset().top;
            var id = $(this).attr("data-id");
            $("#qrcode_box").css({"left": left, "top": top + 55}).show()
        }, function () {
            $("#qrcode_box").fadeOut()
        })
    }
...
function sublogin() {
    ...
    $.post(getUrl("Login/check"), {username: email, pwd: pwd}, function (data) {
        if (data.error != "") {
...
function animateShowTip(obj, tip) {
    ...
    obj.animate({top: top, "height": "16px"}, 500)
}

function animateHideTip(obj) {
    var foot = obj.attr("data-foot");
    obj.animate({top: foot, "height": "0"}, 500)
}

function subcomment(id, mtype, pid, pid_sub) {
    // onclick="subcomment({$topics.id}, '2', {$vo.id}, 0)" type="button"
    var pid_common = pid;
    if (pid_sub > 0) {
        pid_common = pid_sub
    }
...
function subSuggest() {
    ...
    $.get(getUrl("Ajax/subSuggest"), {name: name, email: email, message: message}, function (data) {
        if (data == -1) {
            showWindowBox();
...
function isLogin() {
    ...
    $.post(getUrl("Ajax/isLogin"), {mtype: mtype, id: id, code: code, returnUrl: returnUrl}, function (data) {
...
function getMsgNum() {
    $.get(getUrl("Ajax/getMsgNum"), {}, function (data) {
        if (data > 0) {
...
function getCollect(obj, id, mtype) {
    $.get(contro + "Ajax/getCollect", {mtype: mtype, id: id}, function (data) {
        if (data > 0) {
...
function downloadZipBox(is_original, points_type, points) {
    $.get(getUrl("Ajax/downloadZipBox"), {
        ...
        if (data.userid > 0) {
            ...
            showWindowBox();
            $("#windown_box").attr("data-func", "downloadZipBox(" + is_original + ",'" + points_type + "','" + points + "')");
            return false
        }
    }, "json")
}
...
function downloadZip(id, mtype) {
    ...
    $.get(getUrl("Download/zip"), {mtype: mtype, id: id, first: 1}, function (data) {
...
function blurInputLoginArea(obj, is_sub) {
    ...
    if (val == "") {
        ...
        if (time_error == 0 && is_sub != 1) {
            if (type == "username") {
                $.post(getUrl("Ajax/checkUsername"), {username: val}, function (data) {
                    if (data == -1) {
...
function shareWeibo(id, mtype) {
    $.post(getUrl("Index/shareWeibo"), {mtype: mtype, id: id,}, function (data) {
        if (data.ret == -1) {
            showWindowBox();
