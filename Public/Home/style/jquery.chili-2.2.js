(function ($) {
    ...
    $.fn.chili = function (options) {
                            ...
                            if ($.isFunction(step.replacement)) {
                                ...
                                replacement = step.replacement.apply({
                                    x: function () {
                                        ...
                                        var context = {recipe: step.recipe, blockName: step.blockName};
        ...
        function addLineNumbers(el) {
            ...
            var list_items = html.replace(/(<span [^>]+>)((?:(?:&nbsp;|\xA0)<br>)+)(.*?)(<\/span>)/ig, "$2$1$3$4").replace(/(.*?)(<span .*?>)(.*?)(?:<\/span>(?:&nbsp;|\xA0)<br>|<\/span>)/ig, function (all, before, open, content) {
                if (/<br>/i.test(content)) {
                    var pieces = before.split(br);
                    var lastPiece = pieces.pop();
                    before = pieces.join(br);
                    var aux = (before ? before + br : "") + (lastPiece + content).replace(/((.*?)(?:&nbsp;|\xA0)<br>)|(.*)/ig, function (tmp, not_last_line, not_last, last) {
            ...
            }).replace(/(<li>.*?<\/li>)|((.*?)(?:&nbsp;|\xA0)<br>)|(.+)/ig, function (tmp, prev_li, not_last_line, not_last, last) {
        ...
        function revealChars(tmp) {
            return $.map(tmp.split(""), function (n, i) {
